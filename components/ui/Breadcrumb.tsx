...
function Breadcrumb({ itemListElement = [] }: Props) {
  return (
    <ul class="flex flex-row gap-2 items-center w-full text-default dark:text-white">
      <Item name="Home" item="/" />
      {itemListElement.map((item) => (
        <>
          <li class="mt-0.5 text-default dark:text-white">
            <Icon
              id="ChevronRight"
              class="text-default dark:text-white"
              width={16}
              height={16}
              strokeWidth={2}
            />
